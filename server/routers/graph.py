...
def graph_timeline(top_periods: int = Query(20, ge=1, le=100)):
    """前端时间轴使用。返回每个时期下的代表性文物计数。"""
    with mysql_cursor() as cursor:
        cursor.execute(
            "SELECT period AS name, COUNT(*) AS count "
            "FROM artifacts WHERE period <> '' "
            "GROUP BY period ORDER BY count DESC LIMIT %s",
            (top_periods,),
        )
    ...
    try:
        ...
        with driver.session() as session:
            cypher = (
                ...
                "RETURN [n IN nodes(p) | {id: n.id, name: coalesce(n.name, n.title), "
            ...
            return {"found": True, "nodes": r["nodes"], "rels": r["rels"]}
    except Exception as e:
        raise HTTPException(status_code=503, detail={
            "code": 503, "message": f"Neo4j unavailable: {e}"})
